...
func (n *Asset) getHash(ctx *core.Context, file string, w io.Writer) error {
	var (
		src fs.File
		err error
	)
...
func (n *Asset) GetHash(ctx *core.Context) (string, error) {
	h := md5.New()
	for _, file := range n.Files {
		if err := n.getHash(ctx, file, h); err != nil {
			return "", err
		}
	}
...
func (n *Asset) Execute(ctx *core.Context) error {
	...
	for _, file := range n.Files {
		...
		if strings.HasPrefix(file, "@theme/_internal/") {
			...
			files, err := fs.Glob(ctx.Theme, file)
			if err != nil {
				return err
			}
			matchedFiles = files
